...
def random_goal(goals, prob):
    ...
    for i in range(length-1):
        if accum < num and num < accum + prob[i]:
            return goals[i]
        else:
            accum += prob[i]
    else:
...
class BasicGameEnv(discrete.DiscreteEnv):
    """
    ...
    B : block, cannot enter
    G : goal, where the frisbee is located
    The episode ends when you reach the goal.
    You receive a reward of 1 if you reach the goal, and zero otherwise.
    """

    #reset ask where the starting position will be
    #reset the maps will be between
    #random => [0.6, 0.4]
    #agent => []
    #reset(starting =, goal = [0.6, 0.4])
    ...
    def __init__(self, desc=None, map_name="2x5", prob = [0.4, 0.6]):
        ...
        self.true_map = np.asarray(desc, dtype = 'c')
        ...
        super(BasicGameEnv, self).__init__(nS, nA, P, isd)
