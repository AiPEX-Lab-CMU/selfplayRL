...
def play(p1_control='td_vi',p2_control='td_vi',episodes=5000,train=True,load=False):
    ...
    for episode in range(episodes):
        run_episode(p1_control, p2_control, rl_model, envs, done, train, greedy, episode)
    #l = mp.Lock()
    #cpu_count = mp.cpu_count() - 1
    #pool = mp.Pool(cpu_count, initializer=init, initargs=(l,))
    #func = partial(run_episode, p1_control, p2_control, rl_model, env, done, train, greedy,)
    #pool.map(func, [episode for episode in range(episodes)], chunksize = 1)
    #pool.close
