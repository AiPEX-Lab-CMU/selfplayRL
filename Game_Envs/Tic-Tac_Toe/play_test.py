...
def run_episode(p1_control, p2_control, rl_model, envs, done, train, greedy, cpu, lock, episode):
    ...
    while not done:
        ...
        if(train == False):
            ...
            state_val = rl_model.get_state_val(envs.get_attr("legal_states", incides = index).index(state))#rl_model.get_state_val(envs.legal_states.index(state))
        ...
        if player == 1:
            ...
            if(p1_control == 'random'):
                state, mask, reward, done = envs.env_method("step", masked_random_sample(mask), indices = index)#envs.step(masked_random_sample(mask))
            elif(p1_control == 'td_vi'):
                transitions = envs.env_method("get_possible_transitions", state, indices = index)#envs.get_possible_transitions(state)
                state_idx = envs.get_attr("legal_states", incides = index).index(state)
                state, mask, reward, done = envs.env_method("step", rl_model.choose_act(state_idx, transitions, greedy), indices = index)#envs.step(rl_model.choose_act(state_idx,transitions,greedy))
            else:
        ...
        if done:
            winner = envs.env_method("grid_winner", state[1:], indices = index)#envs.grid_winner(state[1:])
            if(train == False):
                envs.env_method("render", mode = None, indicies = index)#env.render(mode=None)
            if winner == 1:
